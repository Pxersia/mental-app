...
function ResourceCard({ recurso, onClick, isDark }) {
  ...
  return (
    <article
      style={{
        background: isDark ? 'rgba(30,30,32,0.72)' : 'rgba(255,255,255,0.7)',
        ...
        boxShadow: isDark ? '0 12px 32px rgba(0,0,0,0.35)' : '0 12px 32px rgba(0,0,0,0.08)',
        transition: 'transform 0.25s ease, box-shadow 0.25s ease',
        position: 'relative'
      }}
      onClick={() => onClick(recurso)}
      onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.boxShadow = isDark ? '0 16px 40px rgba(0,0,0,0.5)' : '0 16px 40px rgba(0,0,0,0.12)'; }}
      onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = isDark ? '0 12px 32px rgba(0,0,0,0.35)' : '0 12px 32px rgba(0,0,0,0.08)'; }}
    >
      {/* Sin overflow:hidden: la píldora "Recomendado" cuelga 12px por debajo
          de la portada y quedaba cortada por la mitad. El recorte de las
          esquinas redondeadas ya lo hace el overflow del <article>. */}
      <div style={{
        ...
        <div style={{
          ...
          background: isDark ? 'rgba(30,30,32,0.92)' : 'white',
      ...
      <div style={{ padding: '34px 24px 24px 24px', display: 'flex', flexDirection: 'column', flexGrow: 1, gap: '14px' }}>
        ...
        <div>
          <h3 style={{ fontSize: '1.05rem', fontWeight: 800, color: 'var(--text-main)', lineHeight: 1.3, margin: '0 0 8px 0' }}>
            {recurso.titulo}
          </h3>
        ...
        <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', lineHeight: 1.6, margin: 0, flexGrow: 1, display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
          {recurso.descripcion}
        </p>
...
function ResourceModal({ recurso, onClose, isDark }) {
  ...
  const C = isDark
    ? {
        ...
        chipAccent: 'text-blue-300 bg-blue-400/15',
        ghost: 'border-white/15 bg-white/5 text-gray-200 hover:bg-white/10',
      }
    : {
        text: 'text-gray-900',
        textSoft: 'text-gray-600',
        textMuted: 'text-gray-500',
        chip: 'bg-black/5 text-gray-800',
        chipAccent: 'text-blue-600 bg-blue-500/10',
        ghost: 'border-black/10 bg-white/80 text-gray-800 hover:bg-black/5',
      };
    ...
    >
      ...
      >
          ...
          <div className="flex flex-col gap-2.5">
            <div className="flex flex-wrap gap-1.5 mb-1">
              {recurso.tags.slice(0, 3).map(t => (
                <span key={t} className={`text-[0.68rem] font-bold uppercase tracking-[0.03em] px-[9px] py-[3px] rounded-full ${C.chipAccent}`}>
                  {t}
                </span>
              ))}
            </div>

            <h2 className={`text-[1.5rem] md:text-[1.7rem] font-extrabold tracking-[-0.03em] leading-[1.15] ${C.text} mb-1`}>
              {recurso.titulo}
            </h2>

            <p className={`text-[0.92rem] ${C.textMuted}`}>
              Por <strong className={C.text}>{recurso.autor}</strong> · {recurso.editorial} · {recurso.anio}
            </p>

            <div className={`flex items-center gap-4 text-[0.86rem] ${C.textMuted} mt-1`}>
            ...
            <div className="flex flex-wrap gap-2.5 mt-auto pt-2">
              ...
              {recurso.url && recurso.url !== '#' && (
                <a
                  ...
                  className={`h-11 px-5 rounded-full inline-flex items-center justify-center gap-2 border ${C.ghost} text-xs font-bold cursor-pointer transition-colors text-decoration-none`}
                >
        ...
        {recurso.tipo === 'podcast' && (
          <div className={`mb-6 w-full rounded-2xl overflow-hidden shadow-md border ${isDark ? 'border-white/10' : 'border-black/10'} bg-black/5`}>
            <iframe
        ...
        {recurso.tipo === 'video' && (
          <div className={`mb-6 w-full aspect-video rounded-2xl overflow-hidden shadow-md border ${isDark ? 'border-white/10' : 'border-black/10'} bg-black`}>
            <iframe
        ...
        <div className="mb-6">
          <h4 className={`text-[0.95rem] font-bold mb-2 ${C.text}`}>Descripción</h4>
          <p className={`text-[0.92rem] leading-[1.65] ${C.textSoft}`}>
            {recurso.descripcion}
          </p>
        </div>

        <div className="mb-6">
          <h4 className={`text-[0.95rem] font-bold mb-3 ${C.text}`}>Ficha del recurso</h4>
          <div
          ...
          >
            <div className="flex flex-col gap-0.5">
              <span className={`text-[0.68rem] uppercase tracking-[0.05em] ${C.textMuted} font-bold`}>Autor</span>
              <span className={`text-[0.85rem] ${C.text} font-semibold`}>{recurso.autor}</span>
            </div>
            <div className="flex flex-col gap-0.5">
              <span className={`text-[0.68rem] uppercase tracking-[0.05em] ${C.textMuted} font-bold`}>Editorial</span>
              <span className={`text-[0.85rem] ${C.text} font-semibold`}>{recurso.editorial}</span>
            </div>
            <div className="flex flex-col gap-0.5">
              <span className={`text-[0.68rem] uppercase tracking-[0.05em] ${C.textMuted} font-bold`}>Formato</span>
              <span className={`text-[0.85rem] ${C.text} font-semibold`}>{config.label}</span>
            </div>
            <div className="flex flex-col gap-0.5">
              <span className={`text-[0.68rem] uppercase tracking-[0.05em] ${C.textMuted} font-bold`}>Acceso</span>
              <span className={`text-[0.85rem] ${C.text} font-semibold`}>{recurso.gratis ? "Gratuito" : "De pago"}</span>
            </div>
          </div>
        </div>

        <div>
          <h4 className={`text-[0.95rem] font-bold mb-2 ${C.text}`}>Etiquetas</h4>
          <div className="flex flex-wrap gap-2">
            {recurso.tags.map(t => (
              <span key={t} className={`${C.chip} text-[0.68rem] font-bold uppercase tracking-[0.03em] px-[9px] py-[3px] rounded-full`}>
                {t}
              </span>
            ))}
          </div>
...
export default function ResourceLibrary() {
  ...
  /* En móvil el panel de filtros se comería la pantalla entera antes del
     primer recurso, así que arranca plegado y se abre con "Ver filtros". */
    ...
    <>
      <section
        ...
        style={{
          position: 'relative',
          overflow: 'hidden',
          /* El header fijo mide ~97px: con 100px de padding el eyebrow queda
             pegado a la barra. El clamp deja aire real sin estirar el hero. */
          padding: 'clamp(124px, 14vh, 152px) clamp(16px, 5vw, 20px) clamp(40px, 6vh, 64px)',
          display: 'flex',
          flexDirection: 'column',
          alignItems: 'center',
          gap: '24px',
      ...
      >
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: '860px', width: '100%', margin: '0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: '0' }}>
          <div style={{
            display: 'inline-flex', alignItems: 'center', gap: '8px', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-muted)', background: isDark ? 'rgba(30,30,32,0.55)' : 'rgba(255, 255, 255, 0.5)', padding: '6px 16px', borderRadius: '100px', border: isDark ? '1px solid rgba(255,255,255,0.12)' : '1px solid rgba(255, 255, 255, 0.8)', marginBottom: '24px'
          }}>
            <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: BRAND.orange }} />
            Biblioteca · Material clínico · Autoayuda
          </div>

          <h1 style={{ fontSize: 'clamp(2rem, 4.4vw, 3.6rem)', fontWeight: 700, letterSpacing: '-0.04em', lineHeight: 1.05, margin: '0 0 20px 0', color: 'var(--text-main)' }}>
          ...
          <div style={{ position: 'relative', width: '100%', maxWidth: '640px', margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', background: isDark ? 'rgba(30,30,32,0.72)' : 'rgba(255, 255, 255, 0.6)', border: isDark ? '1px solid rgba(255,255,255,0.12)' : '1px solid rgba(255, 255, 255, 0.9)', borderRadius: '100px', padding: '6px 8px 6px 22px', backdropFilter: 'blur(30px) saturate(160%)', WebkitBackdropFilter: 'blur(30px) saturate(160%)', boxShadow: isDark ? '0 24px 48px rgba(0,0,0,0.35)' : '0 24px 48px rgba(0, 0, 0, 0.06), inset 0 1px 0 rgba(255, 255, 255, 1)', transition: 'box-shadow 0.25s ease, border-color 0.25s ease' }}>
              ...
              {busqueda && (
                <button onClick={() => setBusqueda('')} style={{ width: '34px', height: '34px', borderRadius: '50%', border: 'none', background: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0, 0, 0, 0.06)', color: 'var(--text-main)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, transition: 'background 0.2s ease' }} onMouseEnter={e => e.currentTarget.style.background = isDark ? 'rgba(255,255,255,0.16)' : 'rgba(0,0,0,0.12)'} onMouseLeave={e => e.currentTarget.style.background = isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.06)'}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '12px', marginTop: '18px' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px', background: isDark ? 'rgba(30,30,32,0.55)' : 'rgba(255, 255, 255, 0.55)', border: isDark ? '1px solid rgba(255,255,255,0.12)' : '1px solid rgba(255, 255, 255, 0.8)', padding: '8px 18px', borderRadius: '100px', backdropFilter: 'blur(14px)' }}>
      ...
      <section className="relative px-4 sm:px-5 pt-6 pb-16 lg:pt-11 lg:pb-24">
        ...
        <div className="page-body-inner" style={{ maxWidth: '1280px', margin: '0 auto', width: '100%' }}>
          ...
          {/* Bajo 1024px no caben 250px de filtros + una tarjeta de 260px en una
              fila, así que la barra lateral pasa a apilarse sobre la grilla. */}
          <div className="flex w-full flex-col gap-6 lg:flex-row lg:items-start lg:gap-10">

            <aside className={`${sidebarOpen ? 'block' : 'hidden'} w-full lg:sticky lg:top-[120px] lg:w-[260px] lg:shrink-0`}>
              <div
                className="lg:max-h-[calc(100vh-160px)] lg:overflow-y-auto"
                style={{
            ...
            <div className="w-full min-w-0 flex-1">
              ...
              {recursosFiltered.length === 0 ? (
                ...
                <div style={{
                  // El min() evita que el ancho mínimo de tarjeta desborde la
                  // columna cuando el viewport es más estrecho que la tarjeta.
                  display: 'grid',
