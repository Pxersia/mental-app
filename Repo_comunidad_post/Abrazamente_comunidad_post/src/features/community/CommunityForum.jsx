...
export default function CommunityForum() {
  ...
  const API_BASE_URL = 'https://mental-app-backend-3joc.onrender.com';

  const fetchCommunityData = () => {
    fetch(`${API_BASE_URL}/api/foros`)
    ...
    fetch(`${API_BASE_URL}/api/foros/temas/recientes`)
        .then(res => res.ok ? res.json() : [])
        .then(data => {
          if (Array.isArray(data) && data.length > 0) {
            const apiPosts = data.map(t => ({
              ...
              time: new Date(t.fechaCreacion).toLocaleDateString(),
              text: `${t.titulo}\n\n${t.contenido || ''}`,
              likes: t.vistas || 5,
              liked: false,
              comments: []
        ...
        .catch(err => console.warn('Error al cargar publicaciones:', err));
  };
  ...
  async function handleCreatePost(e) {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/foros/temas`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          titulo: composerAnon ? 'Publicación Anónima' : `Publicación de ${user?.nombres || 'Tú'}`,
          contenido: composerText.trim()
        })
      });
      ...
      if (response.ok) {
        ...
        showToast('Error al publicar. Intenta nuevamente.');
      }
    } catch (error) {
