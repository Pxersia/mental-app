...
INSERT INTO recurso_digital (id, titulo, descripcion, tipo_contenido, autor, url_contenido, duracion_minutos, es_premium, vistas, estado)
SELECT v.id, v.titulo, v.descripcion, v.tipo_contenido, v.autor, v.url_contenido, v.duracion_minutos, v.es_premium, v.vistas, v.estado
FROM (VALUES
...
) AS v(id, titulo, descripcion, tipo_contenido, autor, url_contenido, duracion_minutos, es_premium, vistas, estado)
WHERE NOT EXISTS (SELECT 1 FROM recurso_digital t WHERE t.id = v.id);

-- 4. Asignar Categorías a los Nuevos Recursos
INSERT INTO recurso_categoria (recurso_id, categoria_id)
SELECT v.recurso_id, v.categoria_id
FROM (VALUES
  (13, 1), (13, 3),
  (14, 2), (14, 5),
  (15, 3), (15, 6),
  (16, 1), (16, 4)
) AS v(recurso_id, categoria_id)
WHERE NOT EXISTS (SELECT 1 FROM recurso_categoria t WHERE t.recurso_id = v.recurso_id AND t.categoria_id = v.categoria_id);
